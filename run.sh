...
if [ "${backend}" == "nv" ]; then
    echo "nvidia backend..."
    triton-opt --allocate-shared-memory-nv --convert-triton-gpu-to-llvm -reconcile-unrealized-casts "$INPUT_FILE" > "$OUTPUT_FILE"
else
    echo "amd backend..."
    triton-opt --allocate-shared-memory --convert-triton-amdgpu-to-llvm=arch=gfx942 -reconcile-unrealized-casts "$INPUT_FILE" > "$OUTPUT_FILE"
fi
